  ...
  module Deploy
    module Strategy
      class RsyncWithRemoteCache < Remote
        ...
        default_attribute :local_cache, '.cap_local_cache'
        ...
        def copy_remote_cache
          run("rsync -a --delete #{rsync_exclude} #{repository_cache_path}/ #{configuration[:release_path]}/")
        end
        
        def rsync_command_for(server)
          "rsync #{rsync_options} #{rsync_exclude} --rsh='ssh -p #{ssh_port(server)}' #{local_cache_path}/ #{rsync_host(server)}:#{repository_cache_path}/"
        end
        ...
        def rsync_exclude
          if copy_exclude.empty?
            ...
            exclude = copy_exclude.map { |e| "--exclude=\"#{e}\"" }.join(' ')
          end
